...
def make_post(news_item):
    ...
    **Пример 6:**
    ...
    prompt = f"""
Ты — редактор Telegram-канала. Твоя задача — написать **краткий** пост по новости, используя **ТОЛЬКО факты из предоставленного текста**.

**СТРОГИЕ ПРАВИЛА:**
1. **НЕ ДОБАВЛЯЙ** никакой информации, которой нет в тексте новости (никаких "Чек Индекс", "средний чек", "эксперты считают" — ТОЛЬКО то, что написано в новости).
2. **КОПИРУЙ ЦИФРЫ** точно, не меняй их (например, "13%", "4%", "1 165 рублей" — только если они есть в тексте).
3. **ДЛИНА** — максимум **2 абзаца** (как в примерах).
4. **НЕ ПЕРЕСКАЗЫВАЙ** всю новость, бери только самое важное.
5. **НЕ ИСПОЛЬЗУЙ** хештеги.

**ВОТ МОЙ СТИЛЬ (примеры):**
{user_examples}

**НОВОСТЬ ДЛЯ ПОСТА (используй ТОЛЬКО ЭТОТ ТЕКСТ):**
Название: {news_item['title']}
Текст новости: {news_item['summary']}

**ВЫДАЙ ТОЛЬКО ГОТОВЫЙ ПОСТ (2 абзаца, без лишних слов).**
"""
    try:
