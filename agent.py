...
def make_post(news_item):
    ...
    **Пример 6:**
    ...
    prompt = f"""
Ты — редактор Telegram-канала о ритейле. Твоя задача — написать пост по новости, **точно копируя все цифры и факты** из текста.
...
**ТВОЙ ПОСТ ДОЛЖЕН:**
1. Содержать **2 абзаца** (как в примерах).
2. В первом абзаце — суть события и цифры (скопированные дословно).
3. Во втором абзаце — контекст, причину или прогноз (только то, что есть в новости).
4. **НЕ ДОБАВЛЯТЬ** свои комментарии, оценки или информацию, которой нет в тексте.
5. **НЕ ИСПОЛЬЗОВАТЬ** хештеги.

**ВЫДАЙ ТОЛЬКО ГОТОВЫЙ ПОСТ, БЕЗ ЛИШНИХ СЛОВ И ПОЯСНЕНИЙ.**
"""
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "user", "content": prompt}],
            temperature=0.4,  # Немного повысил для естественности языка
            max_tokens=500    # Вернул достаточно места для 2 полноценных абзацев
        )
